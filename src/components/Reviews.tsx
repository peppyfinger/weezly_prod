      ...
      {showForm && state.user && !submitted && (
        <form onSubmit={handleSubmit} className={`p-4 rounded-xl border space-y-3 ${cardBg}`}>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={`block text-xs font-medium mb-1 ${subText}`}>{t('yourName')}</label>
              <input
                type="text"
                value={author}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-1.5 rounded-lg text-xs font-medium bg-gradient-to-r from-cyan-500 to-blue-600 text-white hover:from-cyan-400 hover:to-blue-500 transition-all duration-200"
            >
              {t('submitReview')}
            </button>
          </div>
            ...
            <div key={review.id} className={`p-3 rounded-xl ${cardBg}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className={`text-sm font-medium ${textColor}`}>{review.author}</p>
                    <p className={`text-xs ${subText}`}>{new Date(review.date).toLocaleDateString()}</p>
                  </div>
                </div>
                {renderStars(review.rating)}
              </div>
              <p className={`text-sm ${subText} mt-2`}>{review.text}</p>
            </div>
          ))}
        </div>
      ) : (
