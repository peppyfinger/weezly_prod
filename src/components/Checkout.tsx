...
import { X, CreditCard, Banknote, Building2, Check, Package } from 'lucide-react';
...
export default function Checkout({ open, onClose }: CheckoutProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className={`w-full max-w-2xl rounded-2xl border shadow-2xl overflow-hidden ${isDark ? 'border-slate-700' : 'border-slate-200'} ${cardBg}`}>
        {/* Header */}
            ...
            <div className="flex items-center gap-2">
              {steps.slice(0, 3).map((s, i) => (
                <React.Fragment key={i}>
                  ...
                  {i < 2 && <div className={`flex-1 h-px ${i + 1 < step ? 'bg-emerald-500' : isDark ? 'bg-slate-700' : 'bg-slate-200'}`} />}
                </React.Fragment>
              ))}
            </div>
          </div>
        ...
        <div className="p-5 overflow-y-auto max-h-[60vh]">
          ...
          {step === 2 && (
            <div className="space-y-3 animate-fade-in-up">
              ...
              {[
                { value: 'card', icon: <CreditCard size={18} />, label: t('creditCard') },
                { value: 'cash', icon: <Banknote size={18} />, label: t('cashOnDelivery') },
                { value: 'online', icon: <Building2 size={18} />, label: t('onlineBanking') },
              ].map(opt => (
                <button
                  key={opt.value}
                    ...
                    return (
                      <div key={item.product.id} className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-slate-800' : 'bg-slate-50'}`}>
                        ...
                        <div className="flex-1">
                          ...
                          <p className={`text-xs ${subText}`}>×{item.quantity} · {item.warrantyLevel !== 'none' ? `${t(('warranty' + item.warrantyLevel.charAt(0).toUpperCase() + item.warrantyLevel.slice(1)) as any)} (+${formatPrice(warrantyPrice)})` : t('noWarranty')}</p>
                        </div>
        ...
        {step < 4 && (
          <div className={`flex items-center justify-between p-5 border-t ${divider}`}>
            ...
            {step < 3 ? (
              ...
              <button onClick={handlePlaceOrder} className="px-6 py-2.5 rounded-xl text-sm font-medium bg-gradient-to-r from-emerald-500 to-cyan-600 text-white transition-all duration-200 hover:from-emerald-400 hover:to-cyan-500">
                {t('placeOrder')}
              </button>
            )}
          </div>
