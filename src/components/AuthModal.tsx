...
import { X, User, Mail, Lock, ArrowRight, UserPlus } from 'lucide-react';
      ...
      <div className={`w-full max-w-md rounded-2xl border shadow-2xl overflow-hidden ${cardBg}`}>
        ...
        <div className={`flex items-center justify-between p-5 border-b ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className={`font-bold ${textColor}`}>{mode === 'login' ? t('loginTitle') : t('registerTitle')}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="text-center pt-2">
            ...
            >
              ...
              <span className="text-cyan-500 font-medium">{mode === 'login' ? t('register') : t('login')}</span>
            </button>
          </div>
        </form>
      </div>
